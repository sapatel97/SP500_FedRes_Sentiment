...
foreach(i = 1:nrow(speeches), .options.snow = opts, .packages = c("tidyverse", "httr", "rvest")) %dopar% {
...
# uncomment above later after names

# test_names <- data.frame("testing")
# test_names$X.testing. <- ""

read_html(speeches[20,]$url) %>%
  html_node("#content") %>%
  html_node("#article") %>%
  html_nodes("p") %>%
  html_text() %>%
 ...
 html_node("speaker") %>%
 html_text()
# pre-allocate a dataframe with 100 rows for example if youre looping 100 times so it doesnt have to reallocate memory
for (i in 1:nrow(speeches)) {
...
sent_tokens <- tokens %>%
  ...
  count(sentiment) %>%
  pivot_wider(values_from = n, names_from = sentiment, values_fill = 0) %>%
